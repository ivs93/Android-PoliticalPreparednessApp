...
class RepresentativeViewHolder(val binding: ItemRepresentativeBinding): RecyclerView.ViewHolder(binding.root) {
    ...
    fun bind(item: Representative) {
        ...
        binding.picture.setImageResource(R.drawable.ic_profile)

        //TODO: Show social links ** Hint: Use provided helper methods
        //TODO: Show www link ** Hint: Use provided helper methods

        binding.picture.setImageResource(R.drawable.ic_profile)
        binding.representative?.official?.channels?.let {
            showSocialLinks(it)
        }
        binding.representative?.official?.urls?.let {
            showWWWLinks(it)
        }
    ...
    private fun showSocialLinks(channels: List<Channel>) {
        ...
        if (!facebookUrl.isNullOrBlank()) { enableLink(binding.facebook, facebookUrl) }

        val twitterUrl = getTwitterUrl(channels)
        if (!twitterUrl.isNullOrBlank()) { enableLink(binding.twitter, twitterUrl) }
    }

    private fun showWWWLinks(urls: List<String>) {
        enableLink(binding.website, urls.first())
    }
